...
- **Org Status:** `gtm-prod` (Org ID: never committed here — handled as configuration, like the other org-specific values in `docs/runbooks/fresh-org-deploy.md` §8; look it up via Setup > Company Information in the target org) is **Production**. `gtm-staging` is the rehearsal org and the home of test package versions. Every destructive change or seed script against `gtm-prod` lands directly on live prospect data.
- _Note: This operational risk will persist until migration to the core Publicis Sapient Salesforce Org is finalized._


### Environments

- **dev** = local git worktrees (`../worktrees/issue-<id>`). No org.
- **`gtm-staging`** = rehearsal and test package versions; large API quota. Its deploy needs `EXCLUDE_DIRS=networks,sites ./scripts/deploy.sh gtm-staging`.
- **`gtm-prod`** = live prospect data.
- **Promotion flow:** build in worktree -> QA validate-only against `gtm-staging` -> merge -> deploy `main` to `gtm-staging` for owner review -> deploy to `gtm-prod` only on the owner's explicit go-ahead.
- No default target-org: always pass an explicit alias. Agents never do real deploys.
...
## 5. Deployment Conventions (`gtm-prod`)
...
- **Guard:** repo scripts refuse the alias `gtm-dev` unless `--allow-production` and `GTM_ALLOW_PRODUCTION=1` are both present; `check-all.sh` requires `--org <alias>` (or `--static`). See `docs/runbooks/api-request-budget.md`.
