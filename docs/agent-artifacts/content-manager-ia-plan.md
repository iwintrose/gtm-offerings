...
- **[Implemented as:]** `gtmContentHome.js`'s `openEditor(offeringKey, templateType, isNew, panel)` takes `isNew` as its own argument (not folded into the `panel` param this section originally spec'd) — it stamps a separate `c__new='1'` state key alongside `c__offering`/`c__template`/`c__panel`, so the one-time creation nudge and the `panel` surface selector (§1.3's `c__panel=settings`, etc.) don't collide.
- `gtmContentManager.js`'s `capturePageRef` sets `@track isNewOffering = true` when it reads `ref.state.c__new === '1'`, then calls `stripNewFromUrl()` to scrub `c__new` from the URL via `history.replaceState` so a refresh doesn't re-show it.
- `gtmContentManager.html` renders a dismissible banner above the rail when `isNewOffering` is true: *"{offeringLabel} is live on the offerings page. Next: build its Story and Configurator pages — use “New page for this offering” from the Content home, or the page picker above."* A "Got it" button calls `handleDismissNewOfferingHint()`, which sets `isNewOffering = false`.
- (Corrected 2026-09-24 to match the actual implementation — the original spec below named `c__panel=new`/`showWelcome`, which QA would not find in the code. Left here struck through rather than deleted so QA's checklist history stays traceable: ~~`c__panel=new` distinguishing this from `c__panel=settings`~~, ~~`@track showWelcome`~~, ~~`showWelcome = false`~~.)
