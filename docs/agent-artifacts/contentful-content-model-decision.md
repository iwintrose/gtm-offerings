...
## 9. Addendum — 2026-09-23: verdict reversed by the owner

**§1's "do not migrate" verdict is overturned by the repo owner.** The
original analysis above is preserved verbatim as the historical record of the
reasoning that led to it; nothing in §§1–8 has been edited. This addendum
records why the owner overturned it and what is — and is not — decided as a
result.

### 9.1 Scope change that the reversal turns on

The owner's framing narrows scope relative to the original issue: **Contentful
would replace `gtmContentManager` as the *authoring* surface only.**
`GTM_Page_Content__c` / `GTM_Page_Section__c` / `GTM_Page_Content_Version__c`
remain the system of record; `GtmPageContentReader` keeps serving every guest
and internal read exactly as it does today, unchanged, from SOQL. Sync is
one-directional, Contentful → Salesforce, via a webhook or scheduled pull. No
guest-context Apex ever calls out to Contentful directly. This is the same
shape §6.2 of the conditional plan above already specifies for the read path,
so nothing here undoes that piece — it makes it the whole design, not a
mitigation.

### 9.2 The four facts from §1, addressed in order

1. **"No external content dependency at guest read time today, and
   introducing one is a one-way door."** Not overturned as a fact — reframed
   as moot under 9.1's scope. Guest reads stay on `GtmPageContentReader`/SOQL.
   Contentful never sits on the guest's synchronous path, so this objection
   does not apply to an authoring-only design. (It would still apply in full
   force to the rejected "serve guests directly from Contentful" shape in
   §4-Q5 — that shape remains rejected.)
2. **"This project already adopted a CMS and could not fully exit it."** Owner
   judgment: the prior exit failure (§2 — `ManagedContent` records/definitions
   that could not be deleted by Apex DML or ConnectApi) was an execution
   mistake specific to that vendor's API, not a structural property of
   "adopting any CMS." Owner's stated plan for this attempt: if it doesn't
   work out, purge fully and redo, rather than leave debris the way the last
   attempt did. The §2 debris cleanup (dead remote site settings, the
   `CMS_Channel_Id__c` field, the misleading `gtmConfigData.js` comment, the
   stale `fresh-org-deploy.md` table row) is still owed regardless of this
   reversal, per §2's own framing.
3. **"The org is production and the content has already been damaged once."**
   Owner confirms this is currently **not applicable**: there is no real
   prospect/client data in `gtm-prod` yet — current client interactions are
   placeholder, not live engagements. §8.3's R1/R2 history and its
   "export-first, reversible, human-gated" rule stand as the process to
   re-apply the moment real prospect data exists; this is a closing window,
   not a permanent exemption, and should be re-checked before any live
   migration is executed.
4. **"The GUS coupling is worse than the issue assumes."** Re-verified against
   current code in this conversation (`GtmAgentTone.cls`,
   `GtmAgentGetConfigState.cls`, plus `GtmAgentApplyConfigUpdate`,
   `GtmAgentConfigActions`, `GtmAgentProxyController`,
   `GtmAgentSettingsController`, `GtmAgentToolSurface`,
   `GtmReadoutAgentSurface`, `GtmAppAgentSurface`): the §5.2(a) mitigation
   ("pre-`runLoop()` hydration") this document already recommended has since
   been applied — `GtmAgentTone.clause()` is now resolved **once per
   conversation turn**, before the tool loop starts, not once per round.
   `GtmAgentGetConfigState`'s `industry-chooser` read is **on-demand only**,
   invoked when the model calls `get_gtm_config_state`, not automatically per
   turn. Under 9.1's design this read stays on `GtmPageContentReader`/SOQL
   regardless — it is not proposed to move to Contentful — so §5.2's "up to
   five callouts per turn" scenario does not arise under this reversal at all.
   The point that content only ever *selects* a hardcoded prompt clause and
   never supplies prompt text directly (§5, `TONE_CLAUSES`) is unaffected and
   remains the anti-injection invariant any future design must preserve.

### 9.3 What this addendum does and does not decide

**Decided:** the go/no-go verdict in §1 is reversed, under the narrower scope
in 9.1 (authoring-only, sync into existing Salesforce objects, guest/GUS read
paths unchanged).

**Not decided, not designed, not started:**
- Content-model mapping from Contentful entries to `GtmContentAddress`'s
  `offering::template::section::field` scheme (§3.2/§6.1).
- Draft/publish lifecycle parity — how a Contentful entry draft reconciles
  with `Draft_Value__c`, `Draft_State__c`, `Pending_Delete__c`, and the
  atomic per-page `publishPage` semantics in §3.4.
- Sync mechanism and auth (webhook vs. scheduled pull; Named Credential setup
  — §6.3 still applies, this repo has never deployed one).
- Permission-set impact of any new correlation field (§6.6/`CLAUDE.md` §6).
- §8.1 (procurement/data-processing review) and §8.2 (the Option A/B editing-UX
  fork) are **not** resolved by this addendum — 9.1 narrows scope to
  authoring-only, which is closer to Option A in §4-Q3, but the fork itself
  (proxy UI inside Salesforce vs. editors working directly in Contentful) is
  still open and still the product owner's call.
- No `force-app/`, `scripts/`, or `data/` change; no org operation of any
  kind. This addendum is a documentation-only edit, per explicit instruction
  to record the reversal and stop.

### 9.4 Relationship to backlog B16

`docs/backlog.md` **B16** (open: decide whether to promote this document to
`docs/architecture/adr/0009-...` or retire it) is unaffected by this
reversal — the promotion question is still open, and this addendum does not
answer it. If anything, a reversed verdict argues for promotion sooner rather
than retirement, since the document is now live guidance rather than a closed
"no."
