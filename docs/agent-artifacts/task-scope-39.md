...
1. **Restructure the 9 chapters into a narrower, sharper flow** — 9 sequential full chapters is a lot for a presentation a rep drives live in a client room. Proposed consolidation to 6:
   ...
   - `proof` (unchanged — this is the component's best asset, keep it central; per the confirmed-only rule, this chapter should state plainly what's been delivered (Medtronic, by name, is fine here since it's a real confirmed engagement) and keep any forward-looking capability framed explicitly as "roadmap"/"in progress," never presented as already shipped)
