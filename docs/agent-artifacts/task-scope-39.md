...
## Reconciliation with the Visual/Brand Mockup (added 2026-09-26, later same day)

While this document was being scoped and revised, a parallel, much more heavily-iterated track of work happened directly with the owner: a full visual/brand mockup of all four surfaces, built and corrected live over many rounds, plus a portable, cross-repo Design System extracted from it. Neither track knew about the other's latest state until now. This section reconciles them so the two together satisfy this issue's "current-state audit vs. proposed-design" deliverable — this document covers content/data-architecture; the mockup covers visual/brand execution.

**Companion artifacts (owner has been reviewing these live):**
- Mockup canvas, all 4 surfaces, clickable/scrollable: https://claude.ai/artifact/UygKojzBEtMTacDYw7R79k
- Portable Design System — real token files + example components, not just written rules: https://claude.ai/artifact/SE8G5KGFet5DDTLLjJYNEK

**§4 item 3 below is now done, not open:** the "no PS style guide exists, needs Architect to formalize" gap is closed. A real brand source was located (PS Brand Guidelines, 04.2026, an external OneDrive doc, not yet in-repo) and turned into the Design System artifact above: Radiant Red `#E90130`; Lexend Deca (display)/Roboto (body)/Roboto Mono (functional); a signature notched-rectangle shape system (built as a real reusable SVG-arc component after reverse-engineering how publicissapient.com actually produces the shape — it's pre-cut image assets there, not a live CSS technique, so this repo's version had to be built from scratch); photography rules (must include red or real tech-in-use, never AI-generated people). Recommend porting the token files into `docs/architecture/` as an in-repo follow-up so this isn't solely a Claude-side artifact.

**Gap this document doesn't cover:** the actual picker UI a rep sees first (`offeringChooser`/`chooseIndustry` LWCs) isn't audited in §2/§3 below — those sections are scoped to the `GTM_Offering__mdt` metadata and the industry-variant *data* mechanisms, not the picker's UI/UX. The mockup's Chooser board is a visual proposal for that UI layer, but no code-level current-state audit of those two LWCs' UI has been done this cycle. Flagging as a real gap, not papering over it.

**Two direct conflicts the owner needs to settle** (everything else here is complementary, not contradictory):

1. **Configurator chapter count.** §4 Proposed Design #1 below proposes consolidating 9 chapters to 6 (merging `partner`+`challenge` and `approach`+`engagement`), with a parallax/glassmorphism effect scoped only to those two merged chapters. The mockup instead kept all 9 chapters distinct, using a different effect placement entirely: a CSS scroll-driven cover-stack (Hero → Partner → Challenge sticking and overlapping in sequence) plus a separate glass-card-over-photo demo pattern on the `proof` chapter only (matching Sapient Slingshot/Bodhi's real pattern) — nothing merged. Two different structural proposals for the same surface, arrived at independently. **My read:** the mockup's version has been through many rounds of live, specific owner correction and is the more battle-tested of the two; the 6-chapter consolidation argument (a rep driving 9 full chapters live in a client room is a lot) is still legitimate and worth keeping on the table. This is a real decision, not a detail — recommend the owner picks one before Architect scopes either.

2. **Proof-content disclosure on Configurator.** §4 Proposed Design #1's `proof` chapter (below) says naming **Medtronic** directly is fine there, since it's a real, confirmed Migration Accelerator engagement. The mockup instead anonymizes all Migration-Accelerator-specific proof in the main proof carousel ("Leading U.S. Retailer... 60–70%", "Top-10 Grocer... $4.1M") and only shows real client names (Pandora, Stride Health) in a separate, clearly-labeled "team credibility, not this migration specifically" strip — based on inferring publicissapient.com's own anonymization convention for its published case studies. **This is a client-relationship/confidentiality call, not a design call** — whether it's acceptable to name Medtronic to a new prospect on a client-facing page belongs to the owner/account team, not to this document or the mockup. Flagging plainly rather than picking a side.

**Specifics the mockup settled through live correction that should hold regardless of which chapter structure is chosen:**
- "Book an environment assessment" must link to the offering's existing Assessment/Instrument flow — never a newly-invented booking mechanic.
- The main nav must never hide on scroll in any chapter layout, because it carries that same CTA.
- GUS chat on Configurator is confirmed **rep-only** by direct code inspection this session (`gtmConfigurator.js`, `showAssistant`: `isRepDirect && !viewOnly` — never renders for a prospect on a shared link). It's an AI copilot that lets the rep reconfigure the page by typing, not a live-human queue and not a client-facing FAQ. Any chapter redesign must preserve this gating rather than assume GUS is prospect-facing.
...
1. **Restructure the 9 chapters into a narrower, sharper flow** — 9 sequential full chapters is a lot for a presentation a rep drives live in a client room. Proposed consolidation to 6: **⚠ See "Reconciliation with the Visual/Brand Mockup" above — the companion mockup instead kept all 9 chapters distinct. This is now a flagged open conflict for the owner, not a settled recommendation.**
   ...
   - `proof` (unchanged — this is the component's best asset, keep it central; per the confirmed-only rule, this chapter should state plainly what's been delivered (Medtronic, by name, is fine here since it's a real confirmed engagement); forward-looking capability must stay honest and clearly distinguishable in substance from what's actually shipped, but should read as a confident, narratively-woven part of the story rather than being called out with an explicit "roadmap"/"in progress" label or disclaimer — the distinction is compliance-accuracy, not visible hedging, and that's a real difference worth Architect/Developer understanding: the rule is about what the copy claims, not about tagging it). **⚠ See "Reconciliation with the Visual/Brand Mockup" above — the companion mockup instead anonymizes this chapter's proof content and only names real clients in a separate general-credibility strip. This is now a flagged open conflict for the owner (a confidentiality call, not a design call), not a settled recommendation.**
...
### Open Questions
- Is there PS-approved brand asset availability (logo files, exact hex values, approved font licensing) to formalize into the proposed brand reference doc, or does this overhaul have to proceed with the wordmark/color rules as currently inferred from code? **✅ Resolved — yes: PS Brand Guidelines (04.2026 PDF) was located and used as the source for the Design System artifact linked in "Reconciliation with the Visual/Brand Mockup" above.**
