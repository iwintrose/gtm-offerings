...
**Two direct conflicts — resolved by the owner, 2026-09-26:**

1. **Configurator chapter count. ✅ Decided: keep 9 distinct chapters (the mockup's structure).** §4 Proposed Design #1 below had proposed consolidating 9 chapters to 6 (merging `partner`+`challenge` and `approach`+`engagement`), with a parallax/glassmorphism effect scoped only to those two merged chapters. The mockup instead kept all 9 chapters distinct, using a different effect placement entirely: a CSS scroll-driven cover-stack (Hero → Partner → Challenge sticking and overlapping in sequence) plus a separate glass-card-over-photo demo pattern on the `proof` chapter only (matching Sapient Slingshot/Bodhi's real pattern) — nothing merged. The owner picked the mockup's 9-chapter structure. §4 Proposed Design #1's consolidation-to-6 proposal below is superseded — Architect should scope against the mockup's 9-chapter structure instead.

2. **Proof-content disclosure on Configurator. ✅ Decided: keep proof anonymized (the mockup's approach).** §4 Proposed Design #1's `proof` chapter (below) had proposed naming **Medtronic** directly, since it's a real, confirmed Migration Accelerator engagement. The mockup instead anonymizes all Migration-Accelerator-specific proof in the main proof carousel ("Leading U.S. Retailer... 60–70%", "Top-10 Grocer... $4.1M") and only shows real client names (Pandora, Stride Health) in a separate, clearly-labeled "team credibility, not this migration specifically" strip — based on inferring publicissapient.com's own anonymization convention for its published case studies. The owner picked the mockup's anonymized approach. §4 Proposed Design #1's "Medtronic, by name, is fine here" guidance below is superseded — Configurator's `proof` chapter should stay anonymized per the mockup.
...
1. ~~**Restructure the 9 chapters into a narrower, sharper flow** — 9 sequential full chapters is a lot for a presentation a rep drives live in a client room. Proposed consolidation to 6:~~ **❌ Superseded 2026-09-26 — see "Reconciliation with the Visual/Brand Mockup" above. Owner decided to keep all 9 chapters distinct, per the mockup. Left in place below for context on the reasoning that was considered, not as an active recommendation.**
   ...
   - `proof` (unchanged — this is the component's best asset, keep it central; forward-looking capability must stay honest and clearly distinguishable in substance from what's actually shipped, but should read as a confident, narratively-woven part of the story rather than being called out with an explicit "roadmap"/"in progress" label or disclaimer — the distinction is compliance-accuracy, not visible hedging, and that's a real difference worth Architect/Developer understanding: the rule is about what the copy claims, not about tagging it. ~~Per the confirmed-only rule, this chapter should state plainly what's been delivered (Medtronic, by name, is fine here since it's a real confirmed engagement).~~ **❌ Superseded 2026-09-26 — see "Reconciliation with the Visual/Brand Mockup" above. Owner decided to keep this chapter's proof anonymized, per the mockup; do not name Medtronic here.**)
