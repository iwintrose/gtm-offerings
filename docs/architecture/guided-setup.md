...
| `sdo_demo_flow` | Rec | Issue #38 (2026-09-26 incident). `FlowDefinitionView` row for `ApiName = 'SDO_Service_Case_Creation'`; absent or `IsActive = false` -> DONE; active -> TODO naming the exact `CANNOT_EXECUTE_FLOW_TRIGGER` symptom. Read failure -> UNKNOWN. Scoped to this one confirmed flow only, not a generic SDO-bundle scan (see `docs/backlog.md` "Later: SDO_Service_Case_Status_Updated" for the named, unconfirmed follow-up flow) | LINK Setup > Flows (new `SETUP_PATHS['flows']`) | Yes | A bundled Salesforce "SDO Service" demo flow can silently block every Case insert, with no error surfaced to anyone. |
| `agentforce_agent` | Rec | Issue #38 (2026-09-26 incident). `GTM_Agent_Settings__c.Agentforce_Agent_Id__c` blank -> TODO; set but the `(Id)` cast fails, or a `BotVersion` query for that `BotDefinitionId` with `Status = 'Active'` finds none -> BLOCKED with `blockedBy = null` (a dangling external reference, not an unmet checklist row -- same null-blockedBy shape as `detectSiteActive`'s "no site exists" branch); resolves to an active `BotVersion` -> DONE; the query itself failing -> UNKNOWN. "Active" lives on `BotVersion.Status`, not a field on `BotDefinition` directly (live-confirmed in `gus-chat-provider-settings.md`) | `sectionLink(i, 'claude-gus', ...)` to the existing GUS Configuration section (also added to the LWC's client-side `SETTINGS_SECTION_BY_KEY` map -- see section 7's "must not touch" list for why that map is a second, non-obvious place a new `sectionLink()` row must be registered) | Yes | GUS's Agentforce integration needs an Agent Id that resolves to a real, active agent before it can be turned on. |
