...
> **Historical — do not run.** The command block below deploys onto
> `GTM_Accelerator1`, which is `DownForMaintenance` and slated for deletion
> under a separate, human-gated ticket. It is kept here only as a record of how
> this site was originally provisioned; it is not a live retrieve/deploy path
> and will fail (or worse, resurrect a site meant to stay dark) if run today.
...
site; this metadata adds a page to it, it does not create it. (Again: this was
true when `GTM_Accelerator1` was the live target and is not a current
instruction — see the note at the top of this file.)
...
1. **Publish the site.** Setup → Digital Experiences → All Sites → *(the
   currently-live site)* → *Builder* → **Publish**. An ExperienceBundle deploy
   updates the site's draft; guests see nothing until it is published. Do this
   after every deploy that touches the site's `experiences/` bundle. (This step
   originally named **GTM Accelerator**/`experiences/GTM_Accelerator1`, which is
   no longer the live target — see the note at the top of this file.)
...
> **Historical URLs.** The `/gtmaccelerator/s/...` paths below reflect where
> this smoke test originally ran, on `GTM_Accelerator1`. Substitute the
> currently-live site's own `/readout` path when actually running this test.
