...
existing `gtmOfferingsSnapshot` component (on the GTM Offerings Home
page, `GTM_Offerings_Home` flexipage; formerly the `GTM_Offerings_Overview` tab), alongside the rep's existing "needs attention" and activity
...
- [ ] `MaConfigWizard` LWC covers all 8 full-walkthrough steps plus the
      Quick Link path, embeddable both in a Lightning modal (Home page)
...
- [ ] Add "+ New Prospect Page" action to `gtmOfferingsSnapshot` /
      `GTM_Offerings_Home` flexipage, opening the wizard in a modal
