...
describe('c-gtm-assessment-questionnaire', () => {
    ...
    it('renders the questions from the pack rather than from a hardcoded list', async () => {
        ...
        expect(getPack).toHaveBeenCalledWith({
            // No default (B11) -- mount() here does not set offeringKey, so
            // this is exactly the un-keyed call an unconfigured host would
            // make. It must go up as an explicit blank, never silently as
        ...
        // filters its rows by Offering_Key__c. No default (B11) -- an un-keyed
        // call goes up as an explicit blank rather than silently as
        // migration-accelerator.
    ...
    // ── Start over (explicit, confirmed, wipes everything) ──────────────────

    describe('start over', () => {
        const KEY = 'ma-assessment-progress-v2:direct';

        it('shows a visible Start over button with a hint, at the 32px control height', async () => {
            const el = await mount();
            await toReadiness(el);
            const btn = one(el, '[data-id="start-over"]');
            expect(btn).not.toBeNull();
            expect(btn.classList.contains('slds-button')).toBe(true);
            expect(btn.textContent).toContain('Start over');
            expect(one(el, '.q-startover').textContent).toContain('Erases all your answers');
        });

        it('opens a confirm that says plainly everything is erased, with Keep my answers focused', async () => {
            const el = await mount();
            await toReadiness(el);
            one(el, '[data-id="start-over"]').click();
            await flush();
            const dlg = one(el, '[role="alertdialog"]');
            expect(dlg.textContent).toContain('Start over and erase everything?');
            expect(dlg.textContent).toContain('cannot be');
            const confirm = one(el, '[data-id="confirm-start-over"]');
            expect(confirm.textContent).toContain('Erase everything and start over');
            expect(confirm.classList.contains('slds-button_destructive')).toBe(true);
            const keep = one(el, '[data-id="keep-answers"]');
            expect(keep.classList.contains('slds-button_neutral')).toBe(true);
            expect(el.shadowRoot.activeElement).toBe(keep);
        });

        it('cancel keeps routing, answers, step and local snapshot', async () => {
            const el = await mount();
            await toReadiness(el);
            answerStep(el);
            await flush();
            ...
            one(el, '[data-id="start-over"]').click();
            await flush();
            one(el, '[data-id="keep-answers"]').click();
            await flush();
            expect(one(el, '[role="alertdialog"]')).toBeNull();
            expect(window.localStorage.getItem(KEY)).toBe(before);
            expect(q(el, 'fieldset').length).toBeGreaterThan(0);
        });

        it('confirm clears snapshot, routing and answers, returns to step 0 without submitting', async () => {
            const el = await mount();
            await toReadiness(el);
            answerStep(el);
            await flush();
            ...
            one(el, '[data-id="start-over"]').click();
            await flush();
            one(el, '[data-id="confirm-start-over"]').click();
            await flush();
            expect(window.localStorage.getItem(KEY)).toBeNull();
            expect(one(el, '.q-title').textContent).toBe('About the move');
            expect(one(el, 'select[data-field="source"]').value).toBe('');
            expect(one(el, 'select[data-field="target"]').value).toBe('');
            expect(q(el, 'fieldset').length).toBe(0);
            expect(submitRequest).not.toHaveBeenCalled();
            // A fresh draft session: the old token is not carried forward.
            expect(one(el, '.q-keep-link')).toBeNull();
        });
    });
});
