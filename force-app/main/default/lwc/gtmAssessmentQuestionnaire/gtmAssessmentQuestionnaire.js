...
export default class GtmAssessmentQuestionnaire extends LightningElement {
     ...
     *
     * No default (B11). This used to default to migration-accelerator on the
     * premise that it was "the only offering that has ever been assessed" --
     * that premise breaks the moment a second offering exists, and silently
     * scoring a second offering's respondent against Migration Accelerator's
     * instrument is exactly the mis-scoring ADR-0007 calls a hard boundary.
     * The two real callers both supply this explicitly: embedded mode
     * (gtmConfigurator.html) passes effectiveOfferingKey, and the one
     * standalone route (GTM_Accelerator1's /assessment page) now sets this
     * design attribute to 'migration-accelerator' explicitly in that page's
     * own componentAttributes rather than relying on a component-level
     * default. A future standalone route for a different offering must set
     * its own value the same way -- there is no implicit fallback left.
     */
    @api offeringKey = '';
    ...
    // ------------------------------------------------------------ start over
    //
    // Resume stays: reopening the same link restores routing/answers from
    // localStorage or the server draft. "Start over" is the explicit, confirmed
    // way OUT of that. It erases everything the respondent has entered in this
    // browser session and begins a fresh draft session. It deliberately does NOT
    // delete the prior server draft rows (the existing purge job removes them)
    // and never creates an assessment request.

    @track confirmingStartOver = false;
    _focusKeepButton = false;

    handleAskStartOver() {
        this.confirmingStartOver = true;
        this._focusKeepButton = true;
    }

    handleKeepAnswers() {
        this.confirmingStartOver = false;
    }
    ...
    renderedCallback() {
        if (this._focusKeepButton) {
            const keep = this.template.querySelector('[data-id="keep-answers"]');
            if (keep) {
                keep.focus();
                this._focusKeepButton = false;
            }
        }
    }
    ...
    eraseEverything() {
        ...
        this.contact = { name: '', email: '', company: '', role: '', context: '' };
        this.bdContext = emptyBdContext();
        this.stepIndex = 0;
        this.pack = null;
        this.draftToken = '';
    ...
    async pushDraft(force) {
        ...
        try {
            ...
            if (handle && handle.resumeToken) {
                ...
                // A rep pre-filling a pageless (Rep_Direct) record's answers
                // internally (issue
                // rep-initiated-assessment-no-page-2-share-and-continue)
                // needs this token to build the prospect's share link
                // (GtmSavedConfigurationController.generateRepDirectShareLink).
                // Harmless for every other host -- the same saveDraft()
                // mechanism runs unchanged either way, this just also tells
