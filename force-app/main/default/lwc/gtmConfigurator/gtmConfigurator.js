...
// Generic fallback label only -- used when no offering identity has resolved
// at all (see effectiveOfferingKey / isUnconfigured). Deliberately not a
// specific offering's name: this component must never present as one
// offering while actually holding another (or no) offering's data.
const OFFERING_LABEL = 'This Offering';
...
export default class GtmConfigurator extends LightningElement {
    ...
    @track _cms = {};
    // No default. An Experience Builder page that hasn't had this design
    // attribute set is an unconfigured page, not silently Migration
    // Accelerator's page -- see effectiveOfferingKey / isUnconfigured.
    @api offeringKey = '';
    ...
    /**
     * True once loadSavedConfiguration() has confirmed this record's own
     * Presentation_Stage__c is 'Rep_Direct' -- a pageless, rep-initiated
     * assessment (GtmSavedConfigurationController.createRepDirectAssessment())
     * that was never given branded page content in the first place (issue
     * rep-initiated-assessment-no-page-2-share-and-continue).
     *
     * Read off the same getPublicConfiguration/getConfiguration call every
     * other saved-record field on this page already comes from -- no new
     * wire, no new round trip. Drives showChapterBody/showAssistant to skip
     * the branded chapters entirely (a Rep_Direct record has no
     * Config_Payload__c to build them from) in favour of
     * showRepDirectQuestionnaire, which renders the questionnaire directly.
     */
    @track isRepDirect = false;
    ...
    /** The ?offering= URL param, read in readUrlParams() -- the front door's
     *  (offeringChooser -> chooseIndustry) way of telling this page which
     *  offering a rep/prospect picked before any record exists to save it
     *  against. See effectiveOfferingKey. */
    @track urlOfferingKey = '';

    @track customizeOpen = false;
    @track bookingOpen = false;
    @track isConfigManager = false;
    /** Tri-state for the isRep wire: 'pending' until it resolves, then
     * 'resolved' (a real answer, true or false) or 'error' (the check itself
     * could not run). Initialises to 'pending' so nothing error-specific can
     * render before the wire has actually failed. */
    @track _isRepCheck = 'pending';
    @track savedRecordId = '';
     ...
     * links). Guests hold class access too and get a computed false back --
     * see GtmViewerContext.cls:16-26, where relying on the guest's inability
     * to *call* was diagnosed as a defect and replaced by computing the
     * answer. Whether the URL happens to carry a company/industry param is
     * irrelevant to who's allowed to edit.
     *
     * The check is tracked as a tri-state ('pending' | 'resolved' | 'error')
     * rather than folded into !!data. A thrown wire and a clean false are the
     * same outcome -- fail closed, isConfigManager stays false -- but not the
     * same situation: a clean false is a real visitor with no link, while an
     * error means the access check could not run at all, which is a
     * misconfiguration and must stop masquerading as a product state. The
     * swallow fixed in the guard class was never fixed in this caller.
     *
     * It initialises to 'pending', never 'error', so the operator diagnostic
     * cannot flash before the wire resolves. */
    @wire(isRep)
    wiredIsConfigManager({ data, error }) {
        if (error) {
            this._isRepCheck = 'error';
            this.isConfigManager = false;
            // eslint-disable-next-line no-console
            console.error(
                'gtmConfigurator: GtmViewerContext.isRep wire failed -- the ' +
                    'viewer access check could not run. Most likely cause is ' +
                    'missing Apex class access for GtmViewerContext on this ' +
                    "user's permission sets.",
                error
            );
        } else if (data === undefined) {
            // The Apex wire fires once with data AND error both undefined
            // before the server answers. That is still 'pending': leave the
            // tri-state at its initial value rather than calling it
            // 'resolved', which would collapse the pending state to zero
            // width and defeat the point of tracking it.
            this.isConfigManager = false;
        } else {
            this._isRepCheck = 'resolved';
    ...
    readUrlParams() {
        ...
        const offeringParam = get('offering');
        this.urlOfferingKey = offeringParam || '';
        ...
        // Same param, read plainly, for one purpose only: wording. A rep who
        // followed an old ?wizard=1 link (a bookmark, or a message sent before
        // the wizard moved into Lightning) arrives here as the site's guest
        // user and gets blocked. Knowing they came that way lets the block say
        // where the wizard actually lives now instead of dead-ending them.
        // Changes no gate and opens no new data path.
        this._cameFromWizard = get('wizard') === '1';
    ...
    async loadSavedConfiguration() {
        ...
        try {
            ...
            // See the isRepDirect doc comment above.
            this.isRepDirect = rec.presentationStage === 'Rep_Direct';
    ...
    get showCustomizeButton() {
        // A Rep_Direct record has no branded page content to customize --
        // no company/industry/accent/payload -- so Gus/Customize never
        // applies to it, same reasoning as showAssistant below.
        return this.isConfigManager && !this.viewOnly && !this.isRepDirect;
    }
     ...
     *
     * Also off for a Rep_Direct record (isRepDirect) -- Gus helps fill in a
     * branded page's fields, and a pageless record has none.
     */
    get showAssistant() {
        return this.isConfigManager && !this._preview && !this.viewOnly && !this.isRepDirect;
    }
     ...
     *
     * An internal Salesforce user used to be exempt here (isConfigManager) --
     * meant as "this is the rep's own preview session", but in practice it
     * waved ANY internal user straight through to a live, unattributed
     * questionnaire on the bare /configurator route, no Contact search, no
     * page, no attribution -- the accidental "direct booking" path (issue
     * rep-initiated-assessment-no-page). That exemption is now closed
     * entirely, not replaced by a second parallel flag: an internal user
     * with no savedRecordId sees the same "you need a link" messaging a
     * guest would (see the copy below pointing at Overview's "New
     * engagement link" / "New assessment (no page)" flows).
     */
    get accessBlocked() {
    ...
    /**
     * Which message the already-blocked panel shows. Gated on the 'error'
     * state SPECIFICALLY -- never on "not resolved true" -- because 'pending'
     * must keep showing the plain message while the wire is in flight.
     * accessBlocked is unchanged and still decides whether the panel renders
     * at all; these two only pick the wording inside it.
     *
     * The string this drives is deliberately operator-neutral: GTM_Guest is
     * assigned to a site's Guest User Profile by hand in Setup, so a site
     * missing that assignment produces a genuine anonymous visitor whose wire
     * errors and who lands here. No class name, permission-set name, org or
     * record detail belongs in the UI -- the console.error carries those.
     */
    get accessCheckFailed() {
        return this._isRepCheck === 'error';
    }

    get accessBlockedPlain() {
        return this._isRepCheck !== 'error' && !this.showLoginPrompt;
    }

    /** Whether this page was reached by an old ?wizard=1 link. Copy only --
     *  it adds a line inside the already-rendered block above, and has no say
     *  in whether that block renders. See readUrlParams(). */
    _cameFromWizard = false;

    get cameFromWizard() {
        return this._cameFromWizard;
    }

    /**
     * A rep who followed a ?wizard=1 link (chooseIndustry's tiles, or an old
     * bookmark) but has not completed "Login with Salesforce" is still the
     * site's guest user for this request -- accessBlocked is correctly true
     * (no savedRecordId, no exemption -- see the comment on accessBlocked
     * above) and stays true. What was missing was any indication that
     * logging in is the way out: the guest saw the same "This page needs a
     * link" copy a genuine prospect would, with no path forward.
     *
     * Reuses the existing _isRepCheck tri-state (issue-99-post-sso-identity)
     * rather than inventing a second state machine: gated on 'error' the
     * same way accessBlockedPlain/accessCheckFailed already are, so a wire
     * failure still shows the operator-neutral "Access check failed" panel,
     * never this one. Does NOT gate on _isRepCheck === 'resolved' /
     * isConfigManager -- cameFromWizard is known synchronously from the URL
     * in readUrlParams(), before the wire answers, and a rep who is not yet
     * authenticated needs to see this immediately, not after a round trip
     * that will only ever confirm what the URL already told us (they are not
     * a confirmed rep on THIS unauthenticated request). If isConfigManager
     * later resolves true this render path no longer matters: the wizard
     * itself opens via maybeAutoOpenWizard(), independent of this panel.
     *
     * Changes nothing about who is let through -- accessBlocked's own gate
     * logic is untouched. This only decides which message the already-
     * blocked panel shows.
     */
    get showLoginPrompt() {
        return this._isRepCheck !== 'error' && this._cameFromWizard;
    }

    /** Where the login prompt's link points. Site-relative, consistent with
     *  offeringsUrl/industryUrl's default paths elsewhere on this component
     *  -- resolves against this site's own base path. Carries startURL so a
     *  rep who completes "Login with Salesforce" lands back on the exact
     *  wizard URL they started from, not just the site root. */
    get loginUrl() {
        try {
            const back = `${window.location.pathname}${window.location.search}`;
            return `/login?startURL=${encodeURIComponent(back)}`;
        } catch (e) {
            return '/login';
        }
    }
    ...
    /**
     * What the questionnaire (and, per the B11 routing amendment, the config
     * wizard) is actually answering/saving against.
     *
     * Three-tier resolution, most-authoritative first:
     *   1. linkOfferingKey  -- a cfgId-resolved saved record's own Offering__c
     *      (ADR-0008 section 3; set in loadSavedConfiguration()).
     *   2. urlOfferingKey   -- the ?offering= param the front door
     *      (offeringChooser -> chooseIndustry) forwarded, for a link that
     *      hasn't been saved yet.
     *   3. offeringKey      -- the Experience Builder page-level default,
     *      which (post-B11) is blank unless an admin explicitly set it.
     *
     * Deliberately NOT assigned back over this.offeringKey: getIndustryProfiles
     * and the getPageLayout calls key off the page property and must keep
     * doing so -- this page's *content* is chosen by the page it is, while
     * the *instrument* is chosen by the link. Only the assessment path and
     * (via gtmConfigurator.html) the config wizard read this.
     */
    get effectiveOfferingKey() {
        return this.linkOfferingKey || this.urlOfferingKey || this.offeringKey;
    }

    /** True when no offering identity has resolved from any of the three
     * tiers above -- the explicit "not configured" signal instead of ever
     * silently rendering as Migration Accelerator's content. */
    get isUnconfigured() {
        return !this.effectiveOfferingKey;
    }

    /** Gate for the chapter-body block (progress bar, CRM strip, cover-through-
     * closing sections, assistant preview, footer). False whenever the
     * "published readout" gate takes over OR no offering identity has
     * resolved -- an unconfigured visit must show ONLY the unconfigured
     * notice, never the marketing chapter body underneath it. */
    get showChapterBody() {
        return !this.showPublishedGate && !this.isUnconfigured && !this.isRepDirect;
    }

    /**
     * The Rep_Direct branch (issue
     * rep-initiated-assessment-no-page-2-share-and-continue): skips every
     * branded chapter (cover/partner/challenge/... -- see showChapterBody)
     * and renders ONLY the questionnaire, directly on the page rather than
     * inside the "Get an environment assessment" overlay -- there is no
     * branded page underneath it to open an overlay ON TOP OF.
     *
     * Gated on exactly the same access checks the branded path already
     * enforces (password gate, expiry, inactive, blocked-with-no-link) so a
     * pageless prospect link is exactly as protected as a branded one; only
     * the content behind the gate differs.
     */
    get showRepDirectQuestionnaire() {
        return (
            this.isRepDirect &&
            !this.accessBlocked &&
            !this.expired &&
    ...
    async checkPasswordGate() {
        ...
        // A one-time preview bypass token, appended ONLY to the wizard's
        // "Preview the page" link (see gtmConfigWizard.js previewHref) for
        // the rep who just set this link's password themselves -- a fresh
        // page load on this site's own domain has no shared browser state
        // with the Lightning tab that created the link, so this is the only
        // signal available on first load. Validated server-side against the
        // same HMAC the password-entry path uses (GtmLinkAuthController),
        // same 30-minute window. On success this feeds the same
        // sessionStorage cache the password-entry path writes, so a
        // same-session refresh behaves identically either way.
        try {
            const url = new URL(window.location.href);
            const saToken = url.searchParams.get('saToken');
            if (saToken) {
                const valid = await validatePreviewToken({
                    recordId: this.savedRecordId,
                    token: saToken
                });
                // Strip it from the visible address bar immediately after
                // redemption either way -- unlike the "Copy the link" URL,
                // this one is a live bypass credential and shouldn't linger
                // in history or get accidentally bookmarked/forwarded while
                // still valid.
                url.searchParams.delete('saToken');
                try {
                    window.history.replaceState(null, '', url.pathname + url.search + url.hash);
                } catch (e) { /* address bar rewrite is best-effort */ }

                if (valid) {
                    this._submissionToken = saToken;
                    this.passwordVerified = true;
                    this._gateCheckPending = false;
                    this._logEvent('Page View');
                    try {
                        window.sessionStorage.setItem(`ma-auth-${this.savedRecordId}`, saToken);
                    } catch (e) { /* sessionStorage not available */ }
                    return;
                }
            }
        } catch (e) { /* malformed URL or token -- fall through to the normal gate */ }
    ...
    // A plain string literal template ('mailto:{crmContactEmail}') here does
    // not interpolate in LWC markup -- mixing static text with a `{}`
    // expression in one attribute value is not supported the way it is in
    // Aura, so it rendered as the literal text "mailto:{crmContactEmail}".
    // A getter that returns the already-built value is what LWC needs.
    get crmContactMailto() {
        return this.crmContactEmail ? `mailto:${this.crmContactEmail}` : '';
    }

    get crmContactUrl() {
        if (!this._crmData || !this._crmData.contactId) return '#';
        return `${this._orgUrl}/lightning/r/Contact/${this._crmData.contactId}/view`;
    }
