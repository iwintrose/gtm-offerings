...
jest.mock(
    '@salesforce/apex/GtmReadoutAgentFiles.getFileList',
    () => ({ default: jest.fn() }),
    { virtual: true }
);
...
describe('c-gtm-readout-assist', () => {
    ...
    it('loads the saved notes into the box', async () => {
        ...
        const element = await mount();
        // Notes are present, so the panel is already open by default — no
        // manual toggle needed (and toggling here would close it).
    ...
    it('fetches the persisted file list for the readout on load', async () => {
        await mount();
        expect(getFileList).toHaveBeenCalledWith({ readoutId: 'a01000000000001' });
    });

    it('renders files fetched from the server without waiting for a new upload', async () => {
        getFileList.mockResolvedValue([
            { id: '069000000000002', title: 'discovery-notes.txt', extension: 'txt', sizeBytes: 120, readable: true }
        ]);
        const element = await mount();
        // Attachments are present, so the panel is already open by default.
        expect(element.shadowRoot.querySelector('.ra-file').textContent).toBe('discovery-notes.txt');
    });

    it('keeps working when the file list cannot be read', async () => {
        getFileList.mockRejectedValue(new Error('nope'));
        const element = await mount();
        await open(element);
        expect(element.shadowRoot.querySelector('c-gtm-agent-chat')).not.toBeNull();
    });
    ...
    it('stays collapsed by default when there are no notes and no attachments', async () => {
        getRepNotes.mockResolvedValue('');
        getFileList.mockResolvedValue([]);
        const element = await mount();
        expect(element.shadowRoot.querySelector('.ra-toggle').label).toBe('Ask GUS');
        expect(element.shadowRoot.querySelector('lightning-textarea.ra-notes')).toBeNull();
    });

    it('defaults open when the draft already has rep notes', async () => {
        getRepNotes.mockResolvedValue('Procurement is the blocker.');
        getFileList.mockResolvedValue([]);
        const element = await mount();
        expect(element.shadowRoot.querySelector('lightning-textarea.ra-notes')).not.toBeNull();
        expect(element.shadowRoot.querySelector('.ra-toggle').label).toBe('Hide GUS');
    });

    it('defaults open when the draft already has attachments', async () => {
        getRepNotes.mockResolvedValue('');
        getFileList.mockResolvedValue([
            { id: '069000000000003', title: 'rfp.pdf', extension: 'pdf', sizeBytes: 4096, readable: false }
        ]);
        const element = await mount();
        expect(element.shadowRoot.querySelector('lightning-textarea.ra-notes')).not.toBeNull();
        expect(element.shadowRoot.querySelector('.ra-toggle').label).toBe('Hide GUS');
