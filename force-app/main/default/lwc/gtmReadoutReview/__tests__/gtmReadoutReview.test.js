...
// What GtmSavedConfigurationController.getSiteHomePageUrl actually returns:
// the community siteUrl + '/s' + '/'. The public readout link is built from
// this, so the test uses the real shape rather than a bare domain.
const SITE_HOME = 'https://example.my.site.com/gtmaccelerator/s/';

describe('c-gtm-readout-review', () => {
    beforeEach(() => {
        getSiteHomePageUrl.mockResolvedValue(SITE_HOME);
    });
    ...
    // Approving is the platform's job now — the approval email, the bell, the
    // Approval History related list. A rep approving their own readout from
    // their own editor is exactly what the approval process replaced, so the
    // button must not exist in any state.
    it('never offers an Approve button in any state', async () => {
    ...
    it('copies the public link in the canonical ?token= query-param form', async () => {
        ...
        expect(href).toBe('https://example.my.site.com/gtmaccelerator/s/readout?token=tok123');
    ...
    it('url-encodes the token rather than interpolating it raw', async () => {
        ...
        expect(element.shadowRoot.querySelector('.rr-live-link').getAttribute('href'))
            .toBe('https://example.my.site.com/gtmaccelerator/s/readout?token=a%2Bb%2Fc%3Dd');
    });
    ...
    it('shows the Published badge and live link straight after publishing', async () => {
        ...
        expect(element.shadowRoot.querySelector('.rr-live-link').getAttribute('href'))
            .toBe('https://example.my.site.com/gtmaccelerator/s/readout?token=tok999');
