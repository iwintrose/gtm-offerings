...
/**
 * The editor for a single readout. As of issue #29 it is embedded inside
 * gtmAssessmentSubmissionView (the consolidated Assessment view, reached via
 * a standard__navItemPage deep-link into GTM_Assessment_Submission_View, the
 * successor to the retired standalone GTM_Readouts_Manager "Readout Editor"
 * tab) rather than being a bare top-level tab itself. This component still
 * resolves its own readoutId directly from CurrentPageReference's c__readout
 * state (mirrors gtmContentManager's deep-link pattern), so it required no
 * change to be reused unmodified inside the new container.
 *
 * Every capability below is unchanged and must stay that way: no functional
 * regression from this move is a stated acceptance criterion.
 *
 * Draft: content is editable, Save Draft is always available. In manager mode
 * (GTM_Readout_Approval_Settings__c.Self_Approval_Enabled__c off, the
 * default) Send for approval routes to the owner's manager via the platform
 * Approval Process, and a rep cannot approve their own readout from here.
 * In self-approve mode (the setting on, toggled from the
 * GTM_Readout_Approval_Settings admin tab) Send for approval is replaced by
 * an inline Approve action for the readout's own owner — selfApproveReadout
 * moves the record straight to Approved and logs the action to
 * GTM_Readout_Approval_Log__c as its audit trail, entirely bypassing the
 * Approval Process for that mode.
 * Pending Approval: content is read-only and the record is locked by the
 * approval process (manager mode only — self-approve mode never produces this
 * state, since it goes Draft -> Approved directly); the only action is Recall
 * request, and the banner names who it is sitting with and what happens next,
 * inline — not a pointer to an email, a bell, or the Approval History related
 * list.
...
export default class GtmReadoutReview extends LightningElement {
    /**
     * When placed on a GTM_Assessment_Request__c record page, the platform
     * sets this automatically. The component uses it to look up the linked
     * readout when no c__readout URL state is present (the standalone-tab path).
     */
    @api recordId;

    /**
     * Set to true by the flexipage property on the AR record page so the review
     * Case link is hidden for BD users. Admins reaching the component via the
     * standalone tab still see it (hideCaseLink defaults to false).
     */
    @api hideCaseLink = false;
    ...
    // Site base for the public link, e.g. "https://<domain>/<site-path>/s".
    // getReadoutBaseUrl() resolves the currently-live site's bare base URL
    // (no page path appended), so the link a rep copies here becomes
    // "<base>/readout?token=..." and points at the same site the rest of
    // the app does.
    @track _siteBaseUrl = '';
    ...
    capturePageRef(ref) {
        ...
        if (id && id !== this.readoutId) {
            this.readoutId = id;
            this.loadReadout();
        } else if (!id && this.recordId && !this.readoutId) {
            // Record-page context: no URL state, resolve the linked readout.
            this._resolveByRequest();
        }
    }
    ...
    /**
     * Called when the component is on a GTM_Assessment_Request__c record page
     * and has no c__readout URL state. Looks up the most-recent linked readout
     * and sets readoutId so loadReadout() can proceed normally.
     */
    _resolveByRequest() {
        getReadoutByRequest({ requestId: this.recordId })
            .then((summary) => {
                if (summary && summary.id) {
                    this.readoutId = summary.id;
                    this.loadReadout();
                } else {
                    // No readout generated yet; stop the spinner.
    ...
    /**
     * Toggle-aware routing state, read straight off the DTO
     * (GtmReadoutController.getReadout -> ReadoutDetail) so this component
     * needs no direct access to GTM_Readout_Approval_Settings__c of its own.
     */
    get selfApprovalEnabled() { return !!(this.readout && this.readout.selfApprovalEnabled); }
    get isOwner() { return !!(this.readout && this.readout.isOwner); }

    get pendingApproverLine() {
        const who = this.readout && this.readout.pendingApprover;
        return who
            ? `Sitting with ${who} for approval. You'll see this status change here once they act on it — no need to check anywhere else.`
            : 'Sitting with your manager for approval. You\'ll see this status change here once they act on it — no need to check anywhere else.';
    }
     ...
     *
     * This is an Aura Experience Builder site (ChatterNetworkPicasso), where a
     * custom page routes on a fixed urlPrefix only — there is no path-parameter
     * route for a standalone custom page (see the site's own
     * experiences/<site>/routes/readout.json, urlPrefix "readout", and
     * docs/runbooks/readout-public-link.md). So the token travels as a query
     * param:
     *
     *   https://<site-domain>/<site-path>/s/readout?token=<token>
     *
    ...
    // Manager mode only -- when self-approve is on, Approve replaces Send for
    // approval entirely rather than the two coexisting.
    get canSubmit() { return this.isDraft && !this.isSaving && !this.selfApprovalEnabled; }
    get canSelfApprove() { return this.isDraft && !this.isSaving && this.selfApprovalEnabled && this.isOwner; }
    ...
    /**
     * Self-approve mode's Draft -> Approved action. Mirrors
     * handleSubmitForApproval's save-then-transition shape (the working edit
     * has to be saved first, since it becomes Approved_Content__c the moment
     * this succeeds), but calls selfApproveReadout instead of
     * submitForApproval -- an entirely separate Apex path that never touches
     * Approval.process().
     */
    handleSelfApprove() {
        if (!this.canSelfApprove) return;
        this.isSaving = true;
        saveDraftContent({ readoutId: this.readoutId, content: this.draftValue })
            .then(() => selfApproveReadout({ readoutId: this.readoutId }))
            .then(() => {
                this.toast('Readout approved', 'Logged for audit. You can publish it now.', 'success');
                return this.loadReadout();
            })
            .catch((err) => {
                this.toast('Could not approve', this.messageFrom(err), 'error');
            })
            .finally(() => { this.isSaving = false; });
    }
