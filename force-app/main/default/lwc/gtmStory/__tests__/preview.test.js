...
maybe('story preview page', () => {
    it('writes a standalone page of the rendered component', async () => {
        ...
        // the compiler does, so the preview shows the real cascade. gtmStory.css
        // no longer imports anything (the brand tokens it used to pull in from
        // c/gtmBrandTokens were folded directly into it, issue #33) so this is
        // a no-op today, but kept generic (with comment-stripping and a `seen`
        // set to guard against a self-referencing docstring) for any future
        // shared-CSS module that comes back.
        const seen = new Set();
        const readCss = (file) => {
