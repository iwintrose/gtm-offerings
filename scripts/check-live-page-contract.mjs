...
const SFX = process.env.SF_GUARD_SFX;
if (!SFX) throw new Error('run via scripts/check-all.sh or `scripts/lib/sfx --session -- node ...`');
if (!org) throw new Error('no explicit org: pass an org alias (the machine default org is never used)');
...
const query = (soql) => {
    ...
    let out;
    try {
        out = execFileSync(SFX, args, { encoding: 'utf8', maxBuffer: 32 * 1024 * 1024, stdio: ['ignore', 'pipe', 'pipe'] });
    } catch (e) {
